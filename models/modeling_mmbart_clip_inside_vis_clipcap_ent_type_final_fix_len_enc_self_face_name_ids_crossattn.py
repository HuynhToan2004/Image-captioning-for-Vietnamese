...
from transformers import MBartConfig, MBartPreTrainedModel, GenerationMixin
from transformers.models.mbart.modeling_mbart import MBartAttention, MBartLearnedPositionalEmbedding, MBartModel
...
_EXPECTED_OUTPUT_SHAPE = [1, 8, 512]
...
class ClipViTFeat(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.vit_backbone.ln_post(x[:, 0, :]) # [1, 50, 512] --> [1, 512] ([CLS])
...
class MBartLearnedPositionalEmbedding(nn.Embedding):
    def __init__(self, num_embeddings: int, embedding_dim: int, offset: int = 2):
        self.offset = offset
...
class MBartAttention(nn.Module):
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        # Chia tensor theo số heads và hoán vị kích thước
    ...
    def forward(
        ...
        """
        hidden_states: tensor với shape (batch, tgt_len, embed_dim)
        key_value_states: tensor cho cross-attention (nếu có)
        past_key_value: cached key/value states (nếu có)
        attention_mask: mask với kích thước (batch, 1, tgt_len, src_len)
        """
        is_cross_attention = key_value_states is not None
        bsz, tgt_len, _ = hidden_states.size()

        # Tính query, scaling theo head_dim
        ...
        attn_weights_reshaped = (
            attn_weights.view(bsz, self.num_heads, tgt_len, src_len) if output_attentions else None
        )
...
class MBartEncoderLayer(nn.Module):
    def __init__(self, config: MBartConfig, visual_feature_dim, text_feature_dim, dim_common, max_ner_type_len, max_ner_type_len_gt, only_image):
        super().__init__()
        ...
        if not only_image:
            ...
            self.self_attn_img_name = MBartAttention(
                embed_dim=self.embed_dim,
                num_heads=config.encoder_attention_heads,
                dropout=config.attention_dropout,
            )
            ...
            self.cross_attn_img_ner = MBartAttention(
                embed_dim=self.embed_dim,
                num_heads=config.encoder_attention_heads,
                dropout=config.attention_dropout,
            )
    ...
    def forward(
        ...
        # (Phần xử lý không đổi – giữ nguyên logic của bạn)
        if layer_idx in fusion_layer:
            ...
            hidden_states_img = F.dropout(hidden_states_img, p=self.activation_dropout, training=self.training)
            hidden_states_img = self._linear_1down(hidden_states_img)
            hidden_states_img = F.dropout(hidden_states_img, p=self.dropout, training=self.training)
            ...
            if not self.only_image:
                ...
                hidden_states_face = F.dropout(hidden_states_face, p=self.activation_dropout, training=self.training)
                hidden_states_face = self._face_down(hidden_states_face)
                hidden_states_face = F.dropout(hidden_states_face, p=self.dropout, training=self.training)
                hidden_states_face = residual + hidden_states_face
                hidden_states_face = self.face_layer_norm(hidden_states_face)

                # Xử lý nhánh NER (giữ nguyên logic)
                bsz, ner_len, hidden_dim = hidden_states_ner.size()
                if ner_len < self.max_ner_type_len:
                    pad_len = self.max_ner_type_len - ner_len
                    pad_tensor = torch.zeros(bsz, pad_len, hidden_dim, device=hidden_states_ner.device, dtype=hidden_states_ner.dtype)
                    hidden_states_ner = torch.cat([hidden_states_ner, pad_tensor], dim=1)
                elif ner_len > self.max_ner_type_len:
                    hidden_states_ner = hidden_states_ner[:, :self.max_ner_type_len, :]

                hidden_states_ner_face = torch.cat((hidden_states_face, hidden_states_ner), dim=1)
                src_len = hidden_states_ner_face.size(1)
                tgt_len = hidden_states_ner.size(1)
                bsz = hidden_states_ner.size(0)
                base_mask = torch.ones(bsz, src_len, device=hidden_states_ner.device, dtype=hidden_states_ner.dtype)
                face_name_mask_cross = _expand_mask(base_mask, hidden_states_ner.dtype, tgt_len=tgt_len)
                ...
                hidden_states_ner_transposed = hidden_states_ner.transpose(1, 2)
                hidden_states_ner_prefix = self.activation_fn(self.ner_map_up(hidden_states_ner_transposed))
                hidden_states_ner_prefix = F.dropout(hidden_states_ner_prefix, p=self.activation_dropout, training=self.training)
                hidden_states_ner_prefix = self.ner_map_down(hidden_states_ner_prefix)
                hidden_states_ner_prefix = F.dropout(hidden_states_ner_prefix, p=self.dropout, training=self.training)
                hidden_states_ner_prefix = hidden_states_ner_prefix.transpose(1, 2)
        ...
        if not self.only_image:
            outputs += (hidden_states_face, hidden_states_ner)
        outputs += (hidden_states_img,)

        return outputs

from transformers import MBartConfig, PreTrainedModel
class MBartAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, key_value_states: torch.Tensor = None,
                past_key_value: Optional[tuple] = None, attention_mask: torch.Tensor = None,
                layer_head_mask: torch.Tensor = None, output_attentions: bool = False) -> tuple:
        is_cross_attention = key_value_states is not None
        bsz, tgt_len, _ = hidden_states.size()
        ...
        if layer_head_mask is not None:
            if layer_head_mask.size() != (self.num_heads,):
                raise ValueError(f"Head mask should be of size {(self.num_heads,)}, but is {layer_head_mask.size()}")
            attn_weights = layer_head_mask.view(1, -1, 1, 1) * attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        attn_weights_reshaped = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) if output_attentions else None
        attn_probs = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
        attn_output = torch.bmm(attn_probs, value_states)
        attn_output = attn_output.view(bsz, self.num_heads, tgt_len, self.head_dim).transpose(1, 2)
        attn_output = attn_output.reshape(bsz, tgt_len, self.embed_dim)
...
class MBartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Self-attention (caching key/value ở vị trí 1,2)
...
class MBartPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        # Giả sử bạn có MBartDecoder và MBartEncoder tương tự BartDecoder/BartEncoder
...
class PretrainedMBartModel(MBartPreTrainedModel):
    def __init_subclass__(self):
        warnings.warn(
            "The class `PretrainedMBartModel` has been depreciated, please use `MBartPreTrainedModel` instead.",
            FutureWarning,
        )
...
class MBartEncoder(MBartPreTrainedModel):
    def __init__(
        ...
        fusion_layer=None,
        dim_common: int = 256,
        face_visual_feature_dim: int = 512,
        img_size: int = 2048,
        prompt_mlp_type: str = "clipcap",
        map_size: list = [192, 256, 64, 16],
        prompt_size: int = 10,
        ...
        text_feature_dim = config.d_model  # 512
        ...
        # Sử dụng MBartEncoderLayer thay vì BartEncoderLayer
        self.layers = nn.ModuleList([
            MBartEncoderLayer(config, visual_feature_dim, text_feature_dim, dim_common, max_ner_type_len, max_ner_type_len_gt, only_image)
        ...
        if prompt_mlp_type == "clipcap":
            self.prompt_mlp = MLPClipCap((512, (512 * prompt_size) // 2, 512 * prompt_size))
        else:
            map_size[0] = 512
            self.prompt_mlp = MLP(map_size, hidden_size=512)
        self.prompt_size = prompt_size
        self.prompt_mlp_type = prompt_mlp_type

        if config.d_model == 1024:
            self.visual_map = nn.Linear(512, 1024)
        
        self.only_image = only_image
        if not only_image:
            self.embed_tokens_ner = nn.Embedding(50267, config.d_model, self.padding_idx)
            # Copy phần trọng số từ embed_tokens cho 50265 token đầu tiên
        ...
        self._linear_1 = nn.Linear(face_visual_feature_dim, self.embed_dim)
        self.embed_dim = config.d_model
    ...
    def forward(
        ...
        if not self.only_image:
            ...
            hidden_states_ner = F.dropout(hidden_states_ner, p=self.dropout, training=self.training)
            face_mask = torch.cat((face_mask, name_mask), dim=1)
            tgt_len_ner = hidden_states_ner.size(1)
            face_name_mask_cross = _expand_mask(face_mask, inputs_embeds.dtype, tgt_len=tgt_len_ner)
        ...
        if self.prompt_mlp_type == "clipcap":
            hidden_states_img = hidden_states_img.reshape(hidden_states_img.size(0), self.prompt_size, 512)
...
class MBartDecoder(MBartPreTrainedModel):
    ...
    def forward(
        ...
        # Lấy input_ids hoặc inputs_embeds
        if input_ids is not None and inputs_embeds is not None:
...
class MBartModel(MBartPreTrainedModel):
    def __init__(
        ...
        self.encoder = MBartEncoder(
            ...
            fusion_layer=enc_fusion_layer,
            dim_common=dim_common,
            img_size=img_size,
            prompt_mlp_type=prompt_mlp_type,
            map_size=map_size,
...
from transformers.models.mbart.modeling_mbart import MBartPreTrainedModel, GenerationMixin


from transformers import MBartPreTrainedModel, GenerationMixin, MBartConfig
...
class MBartForMultiModalGeneration(MBartPreTrainedModel, GenerationMixin):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head.weight"]

    def __init__(self, config: MBartConfig, enc_fusion_layer=None, dim_common=256, img_size=2048, 
                 prompt_mlp_type="clipcap", map_size=[192, 256, 64, 16], prompt_size=10,
                 clip_img_model=None, clip_txt_model=None,  
                 freeze_clip=False,
                 max_ner_type_len=80, max_ner_type_len_gt=20, only_image=False, init_attn_weight=False):
        super().__init__(config)
        self.model = MBartModel(config, enc_fusion_layer, dim_common, img_size, prompt_mlp_type, 
                                map_size, prompt_size, max_ner_type_len, max_ner_type_len_gt, only_image)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)

        self.clip_img_model = clip_img_model
        self.clip_txt_model = clip_txt_model
        
        if freeze_clip:
            if self.clip_img_model is not None:
                for param in self.clip_img_model.parameters():
                    param.requires_grad = False
            if self.clip_txt_model is not None:
                for param in self.clip_txt_model.parameters():
                    param.requires_grad = False
    ...
    def forward(
        ...
        name_mask=None,
        add_ner_ffn=True,
    ) -> Union[Tuple, Seq2SeqLMOutput, dict]:
        ...
        # During generation (eval mode, no labels), return Seq2SeqLMOutput
        ...
        # During training or eval with labels, return custom dict with extra fields
        else:
            return {
                "loss": masked_lm_loss,
                "logits": lm_logits,
...
if __name__ == "__main__":
    # prompt_mlp = MLP((192, (512 * 10) // 2, 512 * 10))
    prompt_mlp = MLP(([192, 256, 64, 16]), hidden_size=512)
    print(prompt_mlp)
    print(get_n_params(prompt_mlp))
    prompt_mlp(torch.randn(16,192,512))
    prompt_mlp_clipcap = MLPClipCap((512, (512 * 10) // 2, 512 * 10))
    print(prompt_mlp_clipcap)
    print(get_n_params(prompt_mlp_clipcap))
    prompt_mlp_clipcap(torch.randn(16, 512))